...
(function (d) {
	d.anythingSlider = function (i, k) {
		...
		a.init = function () {
			...
			a.initialized = !1;
			d.isFunction(b.onBeforeInitialize) &&
				a.$el.on("before_initialize", b.onBeforeInitialize);
			a.$el.trigger("before_initialize", a);
			d(
				'<!--[if lte IE 8]><script>jQuery("body").addClass("as-oldie");</script><![endif]-->'
			)
				.appendTo("body")
				.remove();
			a.$wrapper = a.$el
				...
				.addClass("anythingSlider-" + b.theme);
			...
			if (b.buildStartStop || b.buildNavigation)
				a.$controls.appendTo(
					b.appendControlsTo && d(b.appendControlsTo).length
						? d(b.appendControlsTo)
						: a.$wrapper
				);
			b.buildNavigation &&
				a.$nav.appendTo(
					b.appendNavigationTo && d(b.appendNavigationTo).length
						? d(b.appendNavigationTo)
						: a.$controls
				);
			b.buildStartStop &&
				a.$startStop.appendTo(
					b.appendStartStopTo && d(b.appendStartStopTo).length
						? d(b.appendStartStopTo)
						: a.$controls
				);
			...
			a.outerPad = [
				a.$wrapper.innerWidth() - a.$wrapper.width(),
				a.$wrapper.innerHeight() - a.$wrapper.height(),
			];
			b.playRtl && a.$wrapper.addClass("rtl");
			b.expand &&
				((a.$outer = a.$wrapper.parent()),
				a.$window.css({ width: "100%", height: "100%" }),
				a.checkResize());
			...
			var c =
				"slideshow_paused slideshow_unpaused slide_init slide_begin slideshow_stop slideshow_start initialized swf_completed".split(
					" "
				);
			d.each(
				...
				function (f, e) {
					d.isFunction(b[e]) && a.$el.on(c[f], b[e]);
				}
			);
			d.isFunction(b.onSlideComplete) &&
				a.$el.on("slide_complete", function () {
					setTimeout(function () {
						b.onSlideComplete(a);
					}, 0);
					return false;
		...
		a.updateSlider = function () {
			...
			a.navTextVisible =
				"hidden" !== a.$nav.find("span:first").css("visibility");
			a.$nav.empty();
			a.currentPage = a.currentPage || 1;
			a.$items = a.$el.children();
			a.pages = a.$items.length;
			a.dir = "vertical" === b.mode ? "top" : b.playRtl ? "right" : "left";
			b.showMultiple =
				"vertical" === b.mode ? 1 : parseInt(b.showMultiple, 10) || 1;
			b.navigationSize =
				!1 === b.navigationSize ? 0 : parseInt(b.navigationSize, 10) || 0;
			a.$items
				.find("a")
				.off("focus.AnythingSlider")
				.on("focus.AnythingSlider", function (c) {
					...
					if (
						-1 !== e &&
						(e >= a.currentPage + b.showMultiple || e < a.currentPage)
					)
						a.gotoPage(e), c.preventDefault();
				});
			1 < b.showMultiple &&
				(b.showMultiple > a.pages && (b.showMultiple = a.pages),
				(a.adjustMultiple =
					b.infiniteSlides && 1 < a.pages ? 0 : b.showMultiple - 1));
			a.$controls
				.add(a.$nav)
				.add(a.$startStop)
				.add(a.$forward)
			...
			"fade" !== b.mode &&
				b.infiniteSlides &&
				1 < a.pages &&
				(a.$el.prepend(a.$items.filter(":last").clone().addClass("cloned")),
				1 < b.showMultiple
					? a.$el.append(
							a.$items
								.filter(":lt(" + b.showMultiple + ")")
								.clone()
								.addClass("cloned multiple")
					  )
					: a.$el.append(a.$items.filter(":first").clone().addClass("cloned")),
				a.$el.find(".cloned").each(function () {
					d(this)
						.find("a,input,textarea,select,button,area,form")
			...
			if (b.buildNavigation && 1 < a.pages) {
				...
				a.$items.filter(":not(.cloned)").each(function (m) {
					...
					c =
						'<a class="panel' +
						e +
						(a.navTextVisible ? '"' : " " + b.tooltipClass + '" title="@"') +
						' href="#"><span>@</span></a>';
					d.isFunction(b.navigationFormatter)
						? ((g = b.navigationFormatter(e, d(this))),
						  "string" === typeof g
								? h.html(c.replace(/@/g, g))
								: (h = d("<li/>", g)))
						: h.html(c.replace(/@/g, e));
					h.appendTo(a.$nav).addClass(f).data("index", e);
				});
				a.$nav.children("li").on(b.clickControls, function (c) {
					!a.flag &&
						b.enableNavigation &&
						((a.flag = !0),
						setTimeout(function () {
						...
						a.gotoPage(d(this).data("index")));
					c.preventDefault();
				});
				b.navigationSize &&
					b.navigationSize < a.pages &&
					(a.$controls.find(".anythingNavWindow").length ||
						a.$nav
							.before(
					...
					(a.navWidths = a.$nav
						...
						.map(function () {
							return (
								d(this).outerWidth(!0) +
								Math.ceil(
									parseInt(d(this).find("span").css("left"), 10) / 2 || 0
								)
							);
						})
						.get()),
					(a.navLeft = a.currentPage),
					a.$nav.width(a.navWidth(1, a.pages + 1) + 25),
					a.$controls
						.find(".anythingNavWindow")
						.width(a.navWidth(1, b.navigationSize + 1))
						.end()
						.find(".prev,.next")
						.on(b.clickControls, function (c) {
							a.flag ||
								((a.flag = !0),
								setTimeout(function () {
									a.flag = !1;
								...
								a.navWindow(
									a.navLeft + b.navigationSize * (d(this).is(".prev") ? -1 : 1)
								));
							c.preventDefault();
						}));
			}
		...
		a.navWidth = function (b, f) {
			var e;
			e = Math.min(b, f);
			for (var d = Math.max(b, f), h = 0; e < d; e++)
				h += a.navWidths[e - 1] || 0;
			return h;
		};
		...
		a.buildNextBackButtons = function () {
			...
			a.$back.on(b.clickBackArrow, function (c) {
				b.enableArrows &&
					!a.flag &&
					((a.flag = !0),
					setTimeout(function () {
			...
			a.$forward.on(b.clickForwardArrow, function (c) {
				b.enableArrows &&
					!a.flag &&
					((a.flag = !0),
					setTimeout(function () {
			...
			a.$back.appendTo(
				b.appendBackTo && d(b.appendBackTo).length
					? d(b.appendBackTo)
					: a.$wrapper
			);
			a.$forward.appendTo(
				b.appendForwardTo && d(b.appendForwardTo).length
					? d(b.appendForwardTo)
					: a.$wrapper
			);
		...
		a.buildAutoPlay = function () {
			a.$startStop
				.html("<span>" + (a.playing ? b.stopText : b.startText) + "</span>")
				.on(b.clickSlideshow, function (c) {
					b.enableStartStop &&
						(a.startStop(!a.playing),
						a.makeActive(),
						a.playing && !b.autoPlayDelayed && a.goForward(!0));
					c.preventDefault();
				})
				.on("focusin focusout", function () {
					d(this).toggleClass("hover");
		...
		a.checkResize = function (c) {
			...
			a.resizeTimer = setTimeout(function () {
				var f = a.$outer.width() - a.outerPad[0],
					e =
						("BODY" === a.$outer[0].tagName
							? a.$win.height()
							: a.$outer.height()) - a.outerPad[1];
				if (a.width * b.showMultiple !== f || a.height !== e)
					a.setDimensions(), a.gotoPage(a.currentPage, a.playing, null, -1);
		...
		a.setDimensions = function () {
			var c,
				...
				l =
					1 < b.showMultiple
						? a.width || a.$window.width() / b.showMultiple
						: a.$window.width(),
				j = a.$win.width();
			b.expand &&
				((c = a.$outer.width() - a.outerPad[0]),
				(a.height = f = a.$outer.height() - a.outerPad[1]),
				a.$wrapper.add(a.$window).add(a.$items).css({ width: c, height: f }),
			...
			a.$items.each(function (k) {
				...
				if (b.resizeContents) {
					...
					c = g.width() || a.width;
					/*if(e.length===1&&c>=j){c=e.width()>=j?l:e.width();e.css("max-width",c)}*/ g.css(
						"width",
						c
					);
			...
			a.$el.css(
				"vertical" === b.mode ? "height" : "width",
				"fade" === b.mode ? a.width : h
			);
		};
		a.getDim = function (c) {
			var f,
			...
			if (1 < b.showMultiple)
				for (f = 1; f < b.showMultiple; f++)
					(e += a.panelSize[c + f][0]),
						(d = Math.max(d, a.panelSize[c + f][1]));
			return [e, d];
		};
		a.goForward = function (c) {
			a.gotoPage(
				a[b.allowRapidChange ? "targetPage" : "currentPage"] + b.changeBy,
				c
			);
		};
		a.goBack = function (c) {
		...
		a.gotoPage = function (c, f, e, g) {
			...
			/^[#|.]/.test(c) &&
				d(c).length &&
				(c = d(c).closest(".panel").index() + a.adj);
			if (1 !== b.changeBy) {
				...
				1 > c &&
					(c = b.stopAtEnd
						...
						: h);
				c > a.pages
					? (c = b.stopAtEnd ? a.pages : b.showMultiple > 1 - c ? 1 : (c -= h))
					: c >= h && (c = h);
			}
			if (
				!(1 >= a.pages) &&
				...
				"number" !== typeof c &&
					((c = parseInt(c, 10) || b.startPanel), a.setCurrentPage(c)),
				!f || !b.isVideoPlaying(a))
			)
				(a.exactPage = c),
					c > a.pages + 1 - a.adj &&
						(c = !b.infiniteSlides && !b.stopAtEnd ? 1 : a.pages),
					...
					(g = "undefined" !== typeof g ? g : b.animationTime),
					0 <= g && a.$el.trigger("slide_init", a),
					0 < g && a.slideControls(!0),
					b.buildNavigation && a.setNavigation(a.targetPage),
					!0 !== f && (f = !1),
					...
					setTimeout(function (d) {
						var f,
							h = true;
						b.allowRapidChange &&
							a.$wrapper.add(a.$el).add(a.$items).stop(true, true);
						if (!b.resizeContents) {
							f = a.getDim(c);
							d = {};
						...
						if (b.mode === "fade")
							if (a.$lastPage[0] !== a.$targetPage[0]) {
							...
							d = {};
							d[a.dir] =
								-a.panelSize[b.infiniteSlides && a.pages > 1 ? c : c - 1][2];
		...
		a.endAnimation = function (c, d, e) {
			0 === c
				? (a.$el.css(a.dir, "fade" === b.mode ? 0 : -a.panelSize[a.pages][2]),
				  (c = a.pages))
				: c > a.pages &&
				  (a.$el.css(a.dir, "fade" === b.mode ? 0 : -a.panelSize[1][2]),
				  (c = 1));
			a.exactPage = c;
			a.setCurrentPage(c, !1);
			"fade" === b.mode &&
				a.fadeIt(a.$items.not(":eq(" + (c - a.adj) + ")"), 0, 0);
		...
		a.setCurrentPage = function (c, d) {
			...
			if (!(1 > a.pages || 0 === c || isNaN(c))) {
				...
				b.buildArrows &&
					...
					b.stopAtEnd &&
					(a.$forward[
						c === a.pages - a.adjustMultiple ? "addClass" : "removeClass"
					]("disabled"),
					a.$back[1 === c ? "addClass" : "removeClass"]("disabled"),
					c === a.pages && a.playing && a.startStop());
				if (!d) {
					var e = a.getDim(c);
					a.$wrapper
						.css({ width: e[0], height: e[1] })
						.add(a.$window)
						.scrollLeft(0)
						.scrollTop(0);
					a.$el.css(
						...
						"fade" === b.mode
							? 0
							: -a.panelSize[b.infiniteSlides && 1 < a.pages ? c : c - 1][2]
					);
				}
				a.currentPage = c;
				a.$currentPage = a.$items
		...
		a.makeActive = function () {
			a.$wrapper.hasClass("activeSlider") ||
				(d(".activeSlider").removeClass("activeSlider"),
				a.$wrapper.addClass("activeSlider"));
		};
		a.gotoHash = function () {
			getHashPage();
		};
		a.setHash = function (n) {
			var h = a.win.location.hash;
			if (typeof h !== "undefined") {
				a.win.location.hash =
					h.indexOf("p=") > -1 ? h.replace(/p\=\d+/, "p=" + n) : "p=" + n;
			}
		};
		a.slideControls = function (c) {
			...
			b.toggleControls &&
				a.$controls
					.stop(!0, !0)
					.delay(d)
					[c ? "slideDown" : "slideUp"](b.animationTime / 2)
					.delay(e);
			b.buildArrows &&
				b.toggleArrows &&
				(!a.hovered && a.playing && ((h = 1), (g = 0)),
				a.$forward
					.stop(!0, !0)
					.delay(d)
					.animate(
		...
		a.clearTimer = function (b) {
			a.timer &&
				...
				!b &&
					a.slideshow &&
					(a.$el.trigger("slideshow_stop", a), (a.slideshow = !1)));
		};
		a.startStop = function (c, d) {
			!0 !== c && (c = !1);
			if ((a.playing = c) && !d)
				a.$el.trigger("slideshow_start", a), (a.slideshow = !0);
			b.buildStartStop &&
				(a.$startStop
					.toggleClass("playing", c)
					.find("span")
					.html(c ? b.stopText : b.startText),
				"hidden" === a.$startStop.find("span").css("visibility") &&
					a.$startStop
						.addClass(b.tooltipClass)
						.attr("title", c ? b.stopText : b.startText));
			c
				? (a.clearTimer(!0),
				  (a.timer = a.win.setInterval(function () {
						b.isVideoPlaying(a)
							? b.resumeOnVideoEnd || a.startStop()
							: a.goForward(true);
				  }, b.delay)))
				: a.clearTimer();
		};
	...
	d.fn.anythingSlider = function (i, k) {
		return this.each(function () {
			var a,
				b = d(this).data("AnythingSlider");
			(typeof i).match("object|undefined")
				? b
					? b.updateSlider()
					: new d.anythingSlider(this, i)
				: /\d/.test(i) && !isNaN(i) && b
				? ((a = "number" === typeof i ? i : parseInt(d.trim(i), 10)),
				  1 <= a && a <= b.pages && b.gotoPage(a, !1, k))
				: /^[#|.]/.test(i) && d(i).length && b.gotoPage(i, !1, k);
		});
	};
})(jQuery);
